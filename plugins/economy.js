...
const TROLLS = {
    ...
    rico: [
        ...
        '😤 Sí había plata, solo que no era pa\' ti.',
    ],
    trabajo: [
        '💼 Trabajando como si el bot te pagara seguro.',
    ...
    transfer: [
        ...
        '😂 Dale tu plata a otro, que igual no sabías qué hacer.',
    ...
    mina: [
        ...
        '💎 Ojalá esta dedicación la tuvieras en la vida real.',
    ],
    pesca: [
    ...
    ruleta: [
        '🎡 Giró la ruleta con la confianza de quien no tiene nada que perder.',
...
const WORK_MSGS = [
    (job, e) => `Trabajaste como ${job.emoji} ${job.name} y ganaste *${fmt(e)} Stamps*. El jefe casi te despide pero aquí estás.`,
    (job, e) => `Hiciste tu turno de ${job.emoji} ${job.name}. Te pagaron *${fmt(e)} Stamps* y encima te felicitaron. Raro.`,
    (job, e) => `Como ${job.emoji} ${job.name} hoy la pegaste. *${fmt(e)} Stamps*. Gástalo bien (no lo harás).`,
    (job, e) => `El ${job.emoji} ${job.name} rindió hoy. *${fmt(e)} Stamps*. Disfrútalos antes de apostártelos.`,
    (job, e) => `Otro día sobreviviendo como ${job.emoji} ${job.name}. *${fmt(e)} Stamps* pa\' las deudas.`,
]
...
const plugin = async (m, { conn }) => {
    const { cmd, args, prefix } = parseMessage(m)
...
${bonus > 0 ? `🔥 Bonus racha (×${eco.streak}): *+${fmt(bonus)} Stamps*` : `🔥 Racha actual: *${eco.streak} día*`}
...
> ${global.dev}`)
    ...
    //  💼 WORK
    // ══════════════════════════════════════
    if (['work', 'trabajar', 'laburo'].includes(cmd)) {
        const cd = cooldownLeft(eco.lastWork, 3600000)
        if (cd) return reply(`⏳ Ya trabajaste hace poco.\nDescansa: *${cd}*\n\n_El trabajo dignifica... pero no tanto._`)
...
${WORK_MSGS[rand(0, WORK_MSGS.length - 1)](eco.job, earned)}
${pico ? '\n⛏️ (Pico mejorado aplicado +50%)' : ''}
💰 Saldo: *${fmt(eco.stamps)} Stamps*
...
> ${global.dev}`)
    ...
    if (['minar', 'mine', 'mineria'].includes(cmd)) {
        const cd = cooldownLeft(eco.lastMine, 7200000)
        if (cd) return reply(`⏳ La mina está cerrada.\nVuelve en: *${cd}*\n\n_Dale un descanso a esa espalda virtual._`)

        const pico = eco.inventory.includes('palanca')
        const base = rand(100, 400)

        const eventos = [
            { prob: 0.10, txt: '💎 ¡DIAMANTE! Triplicaste lo minado.', mult: 3   },
            { prob: 0.20, txt: '🥇 ¡ORO! Doble de ganancias.',          mult: 2   },
            { prob: 0.30, txt: '🪨 Mineral normal. Sin sorpresas.',      mult: 1   },
            { prob: 0.20, txt: '💥 Explosión menor. Ganaste menos.',     mult: 0.5 },
            { prob: 0.20, txt: '🕳️ Derrumbe. Saliste con poco.',        mult: 0.3 },
        ]
        let r = Math.random(), acc = 0, ev = eventos[2]
        for (const e of eventos) { acc += e.prob; if (r < acc) { ev = e; break } }

        const total = Math.floor(base * ev.mult * (pico ? 1.5 : 1))
        eco.stamps      += total
...
> ${global.dev}`)
    ...
    if (['pescar', 'fish', 'pesca'].includes(cmd)) {
        const cd = cooldownLeft(eco.lastFish, 5400000)
        if (cd) return reply(`⏳ El mar está cerrado.\nVuelve en: *${cd}*\n\n_Hasta el mar te rechaza._`)
        ...
        eco.stamps += val
        if (val > 0) eco.totalEarned += val
        else eco.totalLost += Math.abs(val)
        eco.lastFish = now()
        await save()
...
> ${global.dev}`)
    ...
    if (['crimen', 'crime', 'delinquir'].includes(cmd)) {
        const cd = cooldownLeft(eco.lastCrime, 14400000)
        if (cd) return reply(`⏳ La policía te está buscando.\nEspera: *${cd}*\n\n_Quédate quieto, criminal._`)
...
> ${global.dev}`)
    ...
    if (['limosna', 'beg', 'pedir'].includes(cmd)) {
        const cd = cooldownLeft(eco.lastBeg, 10800000)
        if (cd) return reply(`⏳ Ya pediste hace poco.\nEspera: *${cd}*\n\n_Hasta la limosna tiene límite._`)
...
> ${global.dev}`)
    ...
    if (['slots', 'tragamonedas', 'slot'].includes(cmd)) {
        const cd = cooldownLeft(eco.lastSlots, 1800000)
        if (cd) return reply(`⏳ Las máquinas están en mantenimiento.\nVuelve en: *${cd}*`)
        ...
        const simbolos = ['💗','⭐','🍒','🎵','🎰','💎','🌸','🔥']
        const s = [rand(0,7), rand(0,7), rand(0,7)].map(i => simbolos[i])

        let mult = 0, resultado = ''
        if (s[0] === s[1] && s[1] === s[2]) {
            if (s[0] === '💎') { mult = 10; resultado = '💎 *JACKPOT DIAMANTE* — ×10 !!!' }
            else if (s[0] === '🌸') { mult = 7; resultado = '🌸 *JACKPOT ZERO TWO* — ×7 !!!' }
            else { mult = 5; resultado = `${s[0]} *TRIPLE* — ×5 !!!` }
        } else if (s[0]===s[1] || s[1]===s[2] || s[0]===s[2]) {
            mult = 1.5; resultado = '✨ *PAR* — ×1.5'
        } else {
...
> ${global.dev}`)
    ...
    if (['ruleta', 'roulette', 'girar'].includes(cmd)) {
        const cd = cooldownLeft(eco.lastRoulette, 1800000)
        if (cd) return reply(`⏳ La ruleta está enfriando.\nVuelve en: *${cd}*`)
